...
//            eventInspectorSaveLastEditController (targetController);
    ...
    void DrawEventPanel ()
    {
        ...
        if (targetState != null) {
            ...
            foreach (MecanimEvent e in displayEvents) {
                eventNames.Add (string.Format (
                    "{3}{0}({1})@{2}",
                    e.functionName,
                    e.parameter,
                    GetRealTimeOfTargetState (targetState, e.normalizedTime).ToString ("0.0000"),
                    e.isEnable ? "" : "[DISABLED]"));
            }
    ...
    private float GetRealTimeOfTargetState (AnimatorState state, float time)
    {
        ...
        if (animationClip != null) {
            ...
            if (blendTree != null) {
                return GetBlendTreeLengthRecursive (blendTree) * time;
            }
        }
        return 0;
    }
